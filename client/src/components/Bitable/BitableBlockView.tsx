...
export default function BitableBlockView({ node, updateAttributes, selected, editor, getPos }: NodeViewProps) {
  ...
  const uploadAttachment = (recordId: string, file: File, requestedFieldId?: string): Promise<boolean> => {
    ...
    return new Promise(resolve => {
      ...
      xhr.onerror = () => markFailed('网络错误，上传失败');
      xhr.onabort = () => markFailed('上传已取消');
  ...
  const onDropFiles = (event: DragEvent, recordId?: string) => {
    ...
    event.stopPropagation();
    setDropActive(false);
    const createdRecordId = recordId ? null : addRecord(files[0].name.replace(/\.[^.]+$/, ''));
    const targetRecordId = recordId || createdRecordId;
    if (!targetRecordId) return;
    void Promise.all(files.map(file => uploadAttachment(targetRecordId, file))).then(results => {
      if (createdRecordId && !results.some(Boolean)) performRemoveRecords([createdRecordId]);
    });
  };
  ...
  const renderKanban = () => {
    ...
    const deleteGroup = (choiceId: string) => {
      ...
      if (choices.length <= 1 || !choices.some(choice => choice.id === choiceId)) return;
      setPendingDeleteGroup({ fieldId: statusField.id, choiceId });
      setGroupMigrationTargetId('');
    };
  ...
  return (
    ...
    >
      ...
      {pendingDeleteGroup && (() => {
        const field = table.fields.find(item => item.id === pendingDeleteGroup.fieldId && item.type === 'single_select');
        const choice = field?.options?.choices?.find(item => item.id === pendingDeleteGroup.choiceId);
        if (!field || !choice) return null;
        const cancel = () => {
          setPendingDeleteGroup(null);
          setGroupMigrationTargetId('');
        };
        return (
          <DeleteKanbanGroupDialog
            groupName={choice.name}
            impact={analyzeSelectChoiceDeletion(table, field.id, choice.id)}
            migrationTargets={(field.options?.choices || []).filter(item => item.id !== choice.id)}
            migrationTargetId={groupMigrationTargetId}
            onMigrationTargetChange={setGroupMigrationTargetId}
            onCancel={cancel}
            onConfirm={() => {
              mutate(current => deleteSelectChoiceWithMigration(
                current,
                field.id,
                choice.id,
                groupMigrationTargetId || undefined,
              ));
              cancel();
            }}
          />
