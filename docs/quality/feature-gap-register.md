...
| FG-003 | Grid 权限 | 以只读文档打开 Grid，编辑单元格或新增记录 | Bitable 只检查视图锁定；应继承文档权限并阻断全部写操作 | [Editor.tsx](../../client/src/components/Editor/Editor.tsx)、[BitableBlockView.tsx](../../client/src/components/Bitable/BitableBlockView.tsx) | 缺失：只读 Bitable E2E | REF-GRID-01 |
...
| FG-005 | HTML 导入 | 导入含 `javascript:` URL、危险 CSS、iframe/object 或 active SVG 的 HTML | 仅移除事件属性，无元素/URL/CSS 白名单；应严格净化并拒绝活动内容 | [documentImporter.ts](../../server/src/documentImporter.ts) | 缺失：恶意 HTML/SVG fixture | REF-IMPORT-01 |
| FG-006 | ZIP 导入 | 上传高压缩比或超多 entry 的 ZIP | 只限制压缩包大小，展开内容整体入内存；应限制 entry、展开总量、压缩比和处理时间 | [documentImporter.ts](../../server/src/documentImporter.ts)、[documents.ts](../../server/src/routes/documents.ts) | 部分：[api.test.ts](../../server/tests/api.test.ts) 仅普通 ZIP | REF-IMPORT-02 |
...
| FG-012 | Slash | 在 URL 路径或普通文本中输入 `/query` | 前 30 字符内最后一个 `/` 可能触发菜单；应只接受块首或合法空白边界 | [Editor.tsx](../../client/src/components/Editor/Editor.tsx)、[slashMenuConfig.ts](../../client/src/components/Editor/menus/slashMenuConfig.ts) | 缺失：inline slash/URL | REF-SLASH-01 |
| FG-013 | Slash | 打开表格、分栏、模板或按钮子菜单后用方向键与 Enter | 键盘事件被消费但无法移动/选择子项；应支持方向键、Enter、Esc 与焦点语义 | [SlashMenu.tsx](../../client/src/components/Editor/menus/SlashMenu.tsx) | 部分：现有测试只用鼠标 | REF-SLASH-02 |
| FG-014 | Slash | 选择图片/文件命令后取消原生文件选择器 | Slash 文本已被删除；应保留或恢复用户输入 | [slashMenuConfig.ts](../../client/src/components/Editor/menus/slashMenuConfig.ts) | 缺失 | REF-SLASH-03 |
...
| FG-021 | 评论锚点 | 对文本评论后删除锚点文本并保存 | 保存成功后永久删除评论；应保留 `anchor_lost` 线程并允许重锚/删除 | [DocumentPage.tsx](../../client/src/components/Layout/DocumentPage.tsx)、[commentDocumentSync.ts](../../client/src/components/Editor/blocks/commentDocumentSync.ts) | 已覆盖错误行为：[comment-threads.spec.ts](../../client/tests/comment-threads.spec.ts) | REF-COMMENT-01 |
| FG-022 | 评论附件 | 在评论输入区选择附件 | 文件 input 无上传/持久化链路；应支持预览、重试、删除和刷新恢复 | [CommentSidebar.tsx](../../client/src/components/Layout/CommentSidebar.tsx)、[documents.ts](../../server/src/routes/documents.ts) | 缺失 | REF-COMMENT-02 |
| FG-023 | 评论恢复 | 令创建、编辑或解决请求失败 | 没有线程级持久错误和重试；应原位保留输入与失败状态 | [DocumentPage.tsx](../../client/src/components/Layout/DocumentPage.tsx)、[CommentSidebar.tsx](../../client/src/components/Layout/CommentSidebar.tsx) | 部分：[comment-threads.spec.ts](../../client/tests/comment-threads.spec.ts) happy path | REF-COMMENT-03 |
...
| FG-025 | 普通表格 | 对含 rowspan/colspan 的表执行行列移动 | 模型静默 no-op；应禁用操作并解释结构保护原因 | [tableInsert.ts](../../client/src/components/Editor/tables/tableInsert.ts)、[FeishuTableOverlay.tsx](../../client/src/components/Editor/tables/FeishuTableOverlay.tsx) | 部分：只测简单行拖动 | REF-TABLE-02 |
...
| FG-038 | 导入取消 | 开始大文件/URL 导入后关闭对话框或切换页面 | 客户端没有用户可控 AbortController；应可取消并清理临时资源 | [documents.ts](../../client/src/api/documents.ts)、[DocumentList.tsx](../../client/src/components/DocumentList/DocumentList.tsx) | 缺失 | REF-IMPORT-04 |
